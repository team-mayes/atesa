...
* The Mean Value Plot
	...
	The other issue visible on this plot is unsmoothness, which itself takes two forms: within a single window, and between windows. Unsmoothness between windows (visualized as an apparent discontinuity between adjacent points on the plot) indicates a sudden change in the free energy at that point that has not been sufficiently resolved. This can sometimes be resolved by adding additional sampling windows between the discontinuous windows.
