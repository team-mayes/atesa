...
``resample``
	...
	A boolean. If True, aimless shooting will NOT be performed, and instead the existing aimless shooting data found in *working_directory* are used to produce new output files based on the settings of the current ATESA job. The primary usage of this option is to add additional CVs to the output files without needing to repeat any simulations. It also produces the output file "as_full_cvs.out", which is used as an input for the umbrella sampling option "us_pathway_restraints_file". Default = False
